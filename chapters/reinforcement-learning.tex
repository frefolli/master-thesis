...
\putimage{figures/markov-chain.png}{An example of three state Markov Chain}{fig:markov-chain}{0.99}
...
This model not only accounts for agents' actions, but also provides an abstraction for expressing the quality of an action or the desirability of a certain state transition. Moreover, it allows the definition of a \textbf{policy} $\pi$ as a mapping from the state set to the action set and the definition of functions for evaluating the optimality of a policy. In particular, a policy $\pi _ o$ is \textit{purely optimal} if $\forall \pi \neq \pi _ o$ holds that the accumulated reward over time of $\pi _ o$ is higher than the one of $\pi$. But how can the accumulated rewards be computed?

\putimage{figures/markov-decision-process.png}{The expansion of the Markov Chain of figure \ref{fig:markov-chain} as a Markov Decision Process. Green transitions release a positive reward, red transitions release a negative one.}{fig:markov-decision-process}{0.99}
