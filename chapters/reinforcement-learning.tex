...
SARSA (State-Action-Reward-State-Action) is an on-policy Temporal Difference (TD) algorithm that updates its action-value function based on the action actually taken by the current policy. The update rule is defined as:
...
Q-Learning is an off-policy TD method that seeks to learn the optimal policy independently of the agent’s actions. The core update rule is:
...
Here, $\theta$ are the parameters of the online network and $\theta^{-}$ are the parameters of a target network, updated periodically to stabilize learning.
DQN also incorporates a separate network used to compute stable targets $y$.
...
where $r(\theta) = \frac{\pi_\theta(s, a)}{\pi_{\theta_{\text{old}}}(s, a)}$ is the probability ratio, and $\hat{A}$ is an estimate of the advantage function. The clipping prevents large policy updates that could degrade performance, striking a balance between exploration and stability.
