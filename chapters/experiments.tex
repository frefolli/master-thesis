...
In this chapter, the research objectives and questions introduced in Chapter \ref{chapter:introduction} are addressed thoroughly and individually, with related experiments and conclusions.

\section{Experimental settings and assumptions}
...
%\putimage{figures/setting-breda.png}{The setting "Breda" with nine intersections.}{fig:setting-breda}{1.0}

\paragraph{Scenario}

The scenario used in this experiments is \textit{Bassini} is inspired by Via Bassini, Milan, and has a two-lane two-way main road and five intersecting one-lane two-way roads.
In Figures \ref{fig:sumo-bassini} and \ref{fig:gmaps-bassini} is shown a comparison of this scenario with its real counterpart in Milan.
Alought they don't perfectly match, the scenario is a good approximation that reflects the features of via Bassini while obtaining a regular shape.

The intersections in this network are assumed to be regulated by traffic lights, which by default use directionally synchronized fixed cycle programs, and the agents enacting these traffic lights will have only two possible actions: one action allows incoming traffic from North and South to enter the intersection (following precedence in case of directional conflicts) while the incoming West-East traffic has to wait, the other action does the same but reversed.

%\putimage{figures/sumo-bassini.png}{The setting "Bassini" with five intersections.}{fig:setting-bassini}{1.0}

\putimagecouple
{\putsubimage{figures/gmaps-bassini.png}{A view of Via Bassini/Via Bonardi, in Milan. Source: Google Maps}{fig:gmaps-bassini}{0.45}{1.0}}
{\putsubimage{figures/sumo-bassini.png}{A view of the scenario "Bassini" with five intersections.}{fig:sumo-bassini}{0.45}{1.0}}

\paragraph{Settings}

Since the scenario is a slice of a wider urban area, such slice is assumed to be tightly connected to neighbor slices as can be seen in Figure \ref{fig:city-slice-of-a-whole}, therefore vehicles are assumed to be able to enter freely in the scenario but to face a "queue" when exiting.
This behavior is obtained by generating datasets with $arrivalSpeed = 0$ so that vehicles simulate a slowdown at simulation borders.

To ensure that the demand generated inside a SUMO simulation reflects exactly the traffic description assigned for that simulation, vehicles are configured to be allowed to enter in the simulation with a maximum of $5$ seconds.
In real life, if vehicles in a certained route are experiencing traffic, they will search for alternative routes, therefore this is a reasonable design choice.

Finally, since SUMO doesn't prevent \gls{gl:jam}[s] from happening (meaning intersection-level blockages, in which each car inside the intersection is waiting for another car inside the intersection), a upper bound on time duration of these \gls{gl:jam}[s] is imposed to $120$ seconds, at the end of which period vehicles will just ignore collisions and unblock the intersection.
Altough this is clearly an unrealistic behavior, it does tries to reflect the fact that also in real life such \gls{gl:jam}[s] can happear but usually they are resolved throught precision manouvers and driver coordination.
By setting different priorities to the lane links of intersection the frequency phenomenon can be reduced but never eliminated, therefore this setting is important to ensure a fair comparison.
...
Since the scenario is synthetic though inspired by a real city slice, also flows will be synthetic.
The \gls{gl:transit} flows are made up of only private vehicles (in particular, cars) and settings will reflect the case of an urban area with one main road and more intersecting side roads.
Moreover, the distribution of vehicles over time will follow the two-peak rush hour model presented in Chapter \ref{chapter:traffic-science} and will provide also abnormal \gls{gl:traffic} conditions.
A powerful assumption is that main/side actually reflect the demand from those directions: there is no hidden disequilibrium.

\paragraph{Execution}

The \gls{gl:dataset}[s] are generated using the \textit{generate-datasets} utility with $seed \; = \; 170701$ and the experiments are run automatically by the \textit{executor} module.
...
\section{Acquiring a baseline}
  ...
  The QLAgent has been trained for $400k$ virtual seconds with a \textbf{monolithic} \gls{gl:dataset} containing each one of the entries of the Traffic Registry. It is then evaluated for another $400k$ virtual seconds using an evaluation \gls{gl:dataset} composed of four possible everyday scenarios comprising both low traffic and high congestions. From a technical standpoint, this complete \gls{gl:dataset} has been generated with the following schema using the \textit{generate-flow/dataset} utility:
...
Each of the basic reward functions (\textit{dwt, as, ql, dql, p}) have been used to train an agent with the same \gls{gl:dataset} and seeds.
...
Each of the basic observation functions (\textit{default, s, d, q}) have been used to train an agent with the same \gls{gl:dataset} and seeds. These configurations have been then evaluated with five runs each (using the same seed for each configuration but a different one for each run) and the results have been collected and analyzed.
...
Both the metric are important to evaluate how much a driver get stuck in traffic \gls{gl:jam}[s], since a lower waiting time score may means an higher frequency acceleration-deceleration cycles.
...
Using a proper training \gls{gl:dataset} is at least as important as choosing the right reward and observation functions.
So far a monolithic \gls{gl:dataset} have been used in experiments.
It is constructed so that the agent while training it can experience all possible types of real-world traffic imaginable.
Such \gls{gl:dataset} can be assemble more easily with synthetic data rather than with real data because it would take a very long time for every single possibility to happear and it would be necessary to shred the collected data to synthesize a complete tape.
...
Curriculum learning can applied in this context to simplify \gls{gl:dataset} construction and hopefully enhancing the agent learning process.
The \textbf{Curriculum Daily} \gls{gl:dataset} contains four episodes of daily traffic which represent the regular traffic on workday (2) and weekdays (2) without any kind of artificial impairments.
The \textbf{Curriculum Daily + Disruption} \gls{gl:dataset} follows the same principle as the last \gls{gl:dataset} but replaces two regular traffic episodes with two episodes in which there is disruption.
...
Experiment results (shown in Figure \ref{fig:exp-2-dat}) suggest that the \textbf{Curriculum Daily + Disruption} \gls{gl:dataset} grants a lower average (accumulated) waiting time while increasing the throughput with higher arrival/departure rates, all at the cost of a lower average vehicle speed. Moreover, an inspection of the metric plots over time revealed that the performances of this \gls{gl:dataset} are more stable than the ones of the \textbf{Curriculum Daily} \gls{gl:dataset} and of the \textbf{Monolithic} \gls{gl:dataset}.

Collected data supports the hypothesis that \textit{Curriculum Daily + Disruption} \gls{gl:dataset} balances the best of both other \gls{gl:dataset}[s].
The stability and easiness from the \textit{Curriculum Daily} \gls{gl:dataset} allows agent to incrementally learn without being permanently drown in negative rewards.
On the other hand, the presence of disruptions as in the \textit{Monolithic} \gls{gl:dataset} allows the agent to learn to adapt to tough traffic conditions.
...
This means that a curriculum learning driven \gls{gl:dataset} allows the agents to generalize more and learn a policy of higher quality (with respect to performance and stability) than a collage-like monolithic \gls{gl:dataset}.
...
\putimage{figures/exp/exp-2-dat.png}{Comparison of Monolithic, Curriculum Daily and Curriculum Daily + Disruption \gls{gl:dataset}[s].}{fig:exp-2-dat}{0.75}
...
In Chapter \ref{chapter:reinforcement-learning} more reinforcement learning have been presented and now the interest is in comparing \textit{simpler} \textbf{tabular} methods with the more \textit{sophisticated} \textbf{neural} methods.
...
In both the experiments, the \gls{gl:dataset} being used is \textit{Curriculum Daily + Disruption}, the observation function is \textit{Density} and the reward function is \textit{DiffWaitingTime}.
...
SUMO-RF allows researches to perform a graphical demo in which the agents and their configuration can be seen in action.
...
Now that there is a clearer picture of the potential of the main reinforcement learning algorithms implemented in SUMO-RF, it is vital to understand if they stand a chance with fixed cycle traffic light programs.
...
Traffic habits change and so do demand, usually driven by socio-economic factors but also by perception change due to drivens experiencing more or less traffic \gls{gl:jam}[s] over time.
Ideally, the system could need to be retrained with updated \gls{gl:dataset} every year, if not every month with small learning slots.
What if a reinforcement learning system could automatically detect performance degradation and schedule a small training session when needed, so that it gets stronger over time?
