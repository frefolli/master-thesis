...
Notably, using the function that share the current phase of traffic lights to neighbours, the loss in terms of waiting time and throughput is fairly significant but is counterbalanced with an increase in the average speed.
An hypothesis to explain such phenomenon is that the synchronization that is created by sharing phase information allows to some vehicles to effectively flow though the network.
Passing more than one intersection per cycle means maintaining a higher speed for more time but also more waiting time for other incoming vehicles.
This is a trade-off which also solutions like Green-Waves have to master.
